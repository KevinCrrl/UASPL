...
if os.path.exists(ruta): # Verifico que la ruta exista porque si no existe y el programa asume que está, da error y adiós inicio del programa
    ...
    from idiomas.espanol import palabras
    print("""No se cargó ningún idioma, se usará español por defecto, para cambiar el idioma o dejar de ver este mensaje, cree el archivo config.json de acuerdo a las instrucciones dadas en https://wiki.archlinux.org/title/Uaspl-bin
No language loaded, Spanish will be used by default, to change the language or stop seeing this message, create the config.json file according to the instructions given at https://wiki.archlinux.org/title/Uaspl-bin""")